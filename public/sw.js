		...
		caches
			...
			.then((cacheNames) =>
				Promise.all(
					cacheNames.map((name) => {
						if (
							![STATIC_CACHE, RUNTIME_CACHE, API_CACHE, IMAGE_CACHE].includes(
								name,
							)
						) {
...
self.addEventListener("fetch", (event) => {
	...
	if (isAPI) {
		const isMetadataEndpoint =
			...
			(url.pathname.includes("/movies") &&
				!url.pathname.match(/\/movies\/[^/]+$/)); // List endpoints, not individual

		if (isMetadataEndpoint) {
			// Stale-while-revalidate for category/metadata endpoints
			event.respondWith(
				staleWhileRevalidate(request, API_CACHE, API_CACHE_MAX_AGE),
			);
		} else {
	...
	if (isSameOrigin && isStyleOrScript) {
		event.respondWith(
			staleWhileRevalidate(request, STATIC_CACHE, 24 * 60 * 60 * 1000),
		); // 24 hours
		return;
	}
});
