...
export function usePornPics() {
  ...
  const transformPicture = (pornpicsImage) => {
    ...
    return {
      ...
      thumbnail: pornpicsImage.thumbnail || pornpicsImage.thumb || pornpicsImage.url,
      imageUrl: pornpicsImage.url || pornpicsImage.imageUrl || pornpicsImage.src,
      ...
      added: pornpicsImage.added || pornpicsImage.createdAt || pornpicsImage.date,
      // Additional metadata
      _source: 'pornpics',
      _raw: pornpicsImage,
    };
  ...
  const getPopularPictures = async (page = 1, options = {}) => {
    ...
    try {
      ...
      if (response && (response.pictures || response.data || Array.isArray(response))) {
        const picturesData = response.pictures || response.data || response;
        pictures.value = picturesData.map(transformPicture).filter(p => p !== null);
        currentPage.value = response.page || page;
        totalCount.value = response.total_count || response.totalCount || pictures.value.length;
        searchQuery.value = '';
