...
export function useSmartQueue() {
  ...
  function analyzeUserBehavior() {
    ...
    const avgDuration = watchTimes.length > 0
  ...
  function predictNextVideos(currentVideo, availableVideos, count = 3) {
    ...
    const scoredVideos = availableVideos.map(video => {
      ...
      if (video.category && userPreferences.value.favoriteCategories.includes(video.category)) {
        score += 15;
      }
      ...
      const history = getHistory();
      if (history.some(item => item.id === video.id || item.id === video._id)) {
        score -= 5;
      }
  ...
  async function preloadVideo(video) {
    ...
    try {
      ...
      if (video.url && (video.url.endsWith('.mp4') || video.url.endsWith('.webm') || video.url.includes('.m3u8'))) {
  ...
  async function preloadPredictedVideos(videos) {
    ...
    const shouldPreload = networkQuality !== 'slow-2g' && networkQuality !== '2g';
