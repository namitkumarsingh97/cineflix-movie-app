...
export function useCreatorStudio() {
	...
	function calculateEngagementRate() {
		...
		const totalEngagement =
			creatorStats.value.totalLikes + creatorStats.value.totalComments;
	...
	function calculateProjectedRevenue() {
		const monthlyAvg =
			revenueData.value.monthly.length > 0
				? revenueData.value.monthly.reduce((a, b) => a + b, 0) /
					revenueData.value.monthly.length
				: 0;
