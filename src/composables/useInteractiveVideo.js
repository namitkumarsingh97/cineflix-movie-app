...
export function useInteractiveVideo() {
	...
	function makeChoice(choicePointId, choiceId) {
		...
		const choicePoint = choicePoints.value.find(
			(cp) => cp.id === choicePointId,
		);
	...
	function getPathSummary() {
		return {
			...
			isComplete:
				...
				videoBranches.value[currentSegment.value]?.isEnding,
		};
	}
	...
	function replayFromChoice(choicePointId) {
		...
		const index = currentPath.value.findIndex(
			(c) => c.choicePointId === choicePointId,
		);
