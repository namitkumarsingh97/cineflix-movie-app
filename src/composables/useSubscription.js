...
export function useSubscription() {
  ...
  async function processPayment(paymentData) {
    try {
      ...
      if (paymentData.paymentMethod === 'stripe' || paymentData.paymentMethod === 'card') {
        // First, create payment intent
        const createResponse = await subscriptionApi.createSubscription({
          planId: paymentData.planId,
          type: paymentData.planId === 'lifetime' ? 'lifetime' : paymentData.planId === 'yearly' ? 'yearly' : 'monthly',
      ...
      if (paymentData.paymentMethod === 'upi' && paymentData.verified) {
        ...
        const subscriptionData = {
          ...
          expiresAt: paymentData.planId === 'lifetime' 
            ? null 
