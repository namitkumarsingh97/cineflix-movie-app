...
export function useEporner() {
	...
	const transformVideo = (epornerVideo) => {
		...
		const isDirectVideo =
			...
			(directVideoUrl.endsWith(".mp4") ||
				...
				(directVideoUrl.includes("/video/") &&
					!directVideoUrl.includes("/embed/")));
		...
		return {
			...
			thumbnail:
				epornerVideo.default_thumb?.src || epornerVideo.thumbs?.[0]?.src,
	...
	const getAdaptiveParams = (basePerPage, baseThumbsize, options) => {
		...
		const perPageAdaptive = Math.min(basePerPage, maxThumbnailsPerPage.value);
		...
		try {
			...
			if (response && response.videos) {
				...
				const calculatedPages =
					totalCount.value > 0 ? Math.ceil(totalCount.value / perPageCount) : 1;
				...
				totalPages.value =
					apiTotalPages > MAX_PAGES
						? MAX_PAGES
						: apiTotalPages || calculatedPages;
	...
	return {
		...
		setIncludeGay: (value) => {
			includeGay.value = value;
		},
		setIncludeLowQuality: (value) => {
			includeLowQuality.value = value;
		},
	};
}
