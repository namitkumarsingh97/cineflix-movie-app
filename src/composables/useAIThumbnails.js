...
export function useAIThumbnails() {
		...
		try {
			...
			clickedVideos.forEach((video) => {
				if (video.category) {
					categories.set(
						video.category,
						(categories.get(video.category) || 0) + 1,
					);
				}
				...
				if (video.title) {
					...
					if (title.includes("close") || title.includes("up")) {
						contentTypes.set(
							"close-up",
							(contentTypes.get("close-up") || 0) + 1,
						);
					}
					if (title.includes("wide") || title.includes("full")) {
						contentTypes.set(
							"wide-shot",
							(contentTypes.get("wide-shot") || 0) + 1,
						);
					}
				}
			});
	 ...
	 */
	function getOptimizedThumbnail(
		video,
		defaultThumbnail,
		networkQuality = "4g",
	) {
		try {
			...
			if (personalizedUrl && quality !== "high") {
				...
				if (personalizedUrl.includes("thumb")) {
					optimizedUrl = personalizedUrl.replace(
						/thumb[^/]*/,
						`thumb${quality}`,
					);
				}
			...
			return {
				...
				loading:
					networkQuality === "2g" || networkQuality === "slow-2g"
						? "lazy"
						: "eager",
			};
		} catch (error) {
