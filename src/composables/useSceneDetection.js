...
export function useSceneDetection() {
  ...
  function detectScenes(video, options = {}) {
    ...
    if (useAI) {
      ...
      scenes.push(...simulateAISceneDetection(video, duration, minSceneDuration, maxScenes));
    } else {
      // Fallback: time-based scene detection
      scenes.push(...timeBasedSceneDetection(video, duration, minSceneDuration));
    }
  ...
  function simulateAISceneDetection(video, duration, minDuration, maxScenes) {
    ...
    const hasForeplay = title.includes('foreplay') || categories.some(c => 
      ['romantic', 'soft', 'teasing'].includes(c.toLowerCase())
    );
    const hasAction = title.includes('action') || categories.some(c => 
      ['hardcore', 'intense', 'rough'].includes(c.toLowerCase())
    );
    const hasClimax = title.includes('climax') || categories.some(c => 
    ...
    if (hasForeplay && currentTime < duration * 0.3) {
      const foreplayEnd = Math.min(currentTime + (duration * 0.2), duration * 0.3);
      scenes.push({
        id: `scene_${sceneIndex++}`,
        ...
        thumbnail: generateSceneThumbnail(video, (currentTime + foreplayEnd) / 2),
      });
  ...
  function analyzeSkipPatterns() {
    ...
    skipPatterns.forEach(pattern => {
      pattern.skippedScenes.forEach(sceneType => {
        if (skipCounts.hasOwnProperty(sceneType)) {
  ...
  function getRecommendedSkipTime(video) {
    ...
    if (scenePreferences.value.autoSkipTo === 'action') {
      const actionScene = scenes.find(s => s.type === 'action' || s.intensity === 'high');
      if (actionScene) {
  ...
  function setPreferences(preferences) {
    ...
    try {
      localStorage.setItem('scenePreferences', JSON.stringify(scenePreferences.value));
    } catch (error) {
