...
export function useBadgeCounts() {
  ...
  const totalCount = computed(() => {
    if (isLoggedIn()) {
      return backendCounts.value.total || watchLaterCount.value + followedStarsCount.value;
    }
  ...
  watch(watchLaterItems, () => {
    if (!isLoggedIn()) {
      backendCounts.value.watchLater = watchLaterItems.value.length;
      backendCounts.value.total = watchLaterItems.value.length + followedStars.value.length;
    }
  }, { deep: true });
