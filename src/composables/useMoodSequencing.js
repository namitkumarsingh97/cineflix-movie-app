...
import { useFavorites } from './useWatchHistory';
...
export function useMoodSequencing() {
  ...
  function detectMood(factors = {}) {
    ...
    Object.keys(moodTypes).forEach(moodKey => {
      ...
      // Time of day match
      if (mood.timeOfDay.includes(timeOfDay) || mood.timeOfDay.includes('any')) {
        score += 3;
      }
  ...
  function generateMoodPlaylist(availableVideos, options = {}) {
    ...
    availableVideos.forEach(video => {
      let score = 0;
      const videoCategories = (video.categories || []).map(c => c.toLowerCase());
      ...
      // Intensity match
      if (moodConfig.intensity === 'high' && videoCategories.some(c => 
        ['hardcore', 'intense', 'rough'].includes(c)
      )) {
        score += 8;
      }
      if (moodConfig.intensity === 'low' && videoCategories.some(c => 
        ['romantic', 'soft', 'sensual'].includes(c)
      )) {
        score += 8;
      }
      ...
      const recentlyWatched = history.slice(0, 5).some(h => 
        h.id === video.id || h.id === video._id
      );
      if (recentlyWatched) {
        score -= 10;
      }
  ...
  function optimizePlaylistFlow(playlist, mood) {
    ...
    while (remaining.length > 0) {
      ...
      const lastCategories = (lastVideo.categories || []).map(c => c.toLowerCase());
      ...
      remaining.forEach((video, index) => {
        ...
        const sharedCategories = lastCategories.filter(c => videoCategories.includes(c));
  ...
  function createMoodMix(availableVideos, options = {}) {
    const {
      initialMood = detectMood(),
      adaptInRealTime = true,
    } = options;

    // Generate initial playlist
    let playlist = generateMoodPlaylist(availableVideos, {
      mood: initialMood,
      maxVideos: 30,
    });
  ...
  function adaptPlaylist(playlist, userBehavior) {
    ...
    const filtered = playlist.filter(video => 
      !skippedVideos.some(skipped => 
        skipped.id === video.id || skipped.id === video._id
      )
    );
    ...
    if (watchedVideos.length > 0) {
      ...
      const remainingVideos = availableVideos.filter(v => 
        !filtered.some(f => f.id === v.id || f.id === v._id)
      );
