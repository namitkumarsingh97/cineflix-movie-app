...
export function useRecommendations() {
    ...
    try {
      ...
      if (sessionData) {
        ...
        if (now - session.startTime < SESSION_DURATION) {
          ...
          session.categories = Array.isArray(session.categories) ? session.categories : [];
          session.stars = Array.isArray(session.stars) ? session.stars : [];
          session.watchedItems = Array.isArray(session.watchedItems) ? session.watchedItems : [];
          session.durations = Array.isArray(session.durations) ? session.durations : [];
          return session;
        }
      }
    } catch (err) {
  ...
  const calculateCFScore = (item, watchHistory) => {
    ...
    watchHistory.forEach((watched) => {
      ...
      const recency = watched.watchedAt 
        ? Math.max(0, 1 - (Date.now() - new Date(watched.watchedAt).getTime()) / (30 * 24 * 60 * 60 * 1000))
        : 0.5;
  ...
  const reRankBySession = (recommendations, session) => {
    ...
    return recommendations.map(item => {
      ...
      const sessionCategories = new Set((session.categories || []).map(c => c.toLowerCase()));
      if (item.categories) {
      ...
      const sessionStars = new Set((session.stars || []).map(s => s.toLowerCase()));
      if (item.stars) {
      ...
      if (session.durations && session.durations.length > 0 && item.duration) {
  ...
  const applyDiversityConstraints = (recommendations, maxPerCategory = 3, maxPerStar = 2) => {
    ...
    const remainingItems = remaining.filter(item => !diversified.includes(item));
    diversified.push(...remainingItems.slice(0, Math.max(0, recommendations.length - diversified.length)));
  ...
  const getBecauseYouWatched = (currentItem, allItems, limit = 12) => {
    ...
    const scored = allItems
      .filter(item => (item.id || item._id) !== (currentItem.id || currentItem._id))
      .map(item => {
  ...
  const getTrendingByContext = (allItems, limit = 12) => {
    ...
    const timeOfDayMultiplier = (item) => {
      ...
      if (hour >= 12 && hour < 18) {
        return item.duration && item.duration >= 1800 && item.duration < 3600 ? 1.2 : 1.0;
      }
    ...
    const scored = allItems.map(item => {
      ...
      // Recency boost
      const recencyBoost = item.createdAt || item.uploadedAt || item.added
        ? Math.max(0, 1 - (Date.now() - new Date(item.createdAt || item.uploadedAt || item.added).getTime()) / (7 * 24 * 60 * 60 * 1000))
        : 0;
      
      // Preference boost
      const preferenceBoost = preferredCategories.length > 0 && item.categories
        ? item.categories.some(cat => preferredCategories.includes(cat.toLowerCase())) ? 0.3 : 0
        : 0;
      ...
      const finalScore = (baseScore + recencyBoost * 0.3 + preferenceBoost + cfBoost) * timeBoost * regionBoost;
