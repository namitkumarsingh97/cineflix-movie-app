...
const MOODS = {
	energetic: {
		keywords: ["action", "intense", "fast", "thrilling"],
		tempo: "fast",
		length: "short",
	},
	relaxed: {
		keywords: ["slow", "romantic", "sensual", "soft"],
		tempo: "slow",
		length: "medium",
	},
	adventurous: {
		keywords: ["outdoor", "public", "adventure", "wild"],
		tempo: "medium",
		length: "medium",
	},
	intimate: {
		keywords: ["couple", "romantic", "private", "close"],
		tempo: "slow",
		length: "long",
	},
	quick: {
		keywords: ["short", "quick", "fast", "brief"],
		tempo: "fast",
		length: "short",
	},
	marathon: {
		keywords: ["long", "extended", "full", "complete"],
		tempo: "medium",
		length: "long",
	},
};
...
export function usePlaylists() {
	...
	const removeFromPlaylist = (playlistId, itemId) => {
		...
		playlist.items = playlist.items.filter((i) => (i.id || i._id) !== itemId);
		playlist.updatedAt = new Date().toISOString();
		savePlaylists(playlists.value);
		return true;
	};
		...
		const scored = allItems
			.map((item) => {
				...
				if (item.duration) {
					...
					if (length === "short" && duration < 1800)
						score += 0.3; // < 30 min
					else if (length === "medium" && duration >= 1800 && duration < 3600)
						score += 0.3; // 30-60 min
