<template>
  <div class="cumshot-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="cumshot-content">
      <div class="cumshot-page">
        <div class="page-header">
          <h1 class="page-title">Cumshot</h1>
          <p class="page-subtitle">Browse all Cumshot videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('cumshot', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('cumshot', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.cumshot-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.cumshot-content { flex: 1; min-width: 0; }
.cumshot-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .cumshot-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .cumshot-page { padding: 24px; } }
@media (max-width: 768px) {
  .cumshot-page { padding: 16px; }
