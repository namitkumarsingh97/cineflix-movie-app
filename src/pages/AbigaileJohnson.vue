...
const displayTotalPages = computed(() => { const total = totalPages.value; if (total > 1000) return '1000+'; return total.toString(); });
const visiblePages = computed(() => { const pages = []; const total = Math.min(totalPages.value, 1000); const current = currentPage.value; if (total <= 7) { for (let i = 1; i <= total; i++) pages.push(i); } else { pages.push(1); if (current > 3) pages.push('...'); const start = Math.max(2, current - 1); const end = Math.min(total - 1, current + 1); for (let i = start; i <= end; i++) pages.push(i); if (current < total - 2) pages.push('...'); pages.push(total); } return pages; });
function navigateToVideo(video) { router.push(`/watch/${video.id}?source=eporner`); }
function goToPage(page) { if (page === '...' || page < 1 || page > totalPages.value) return; searchVideos('abigaile johnson', page, { order: 'most-popular' }); window.scrollTo({ top: 0, behavior: 'smooth' }); }
onMounted(() => { searchVideos('abigaile johnson', 1, { order: 'most-popular' }); });
</script>
<style scoped>
