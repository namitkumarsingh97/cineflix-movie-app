<template>
  <div class="bukkake-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="bukkake-content">
      <div class="bukkake-page">
        <div class="page-header">
          <h1 class="page-title">Bukkake</h1>
          <p class="page-subtitle">Browse all Bukkake videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('bukkake', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('bukkake', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.bukkake-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.bukkake-content { flex: 1; min-width: 0; }
.bukkake-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .bukkake-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .bukkake-page { padding: 24px; } }
@media (max-width: 768px) {
  .bukkake-page { padding: 16px; }
