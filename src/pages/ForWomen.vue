<template>
  <div class="forwomen-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="forwomen-content">
      <div class="forwomen-page">
        <div class="page-header">
          <h1 class="page-title">For Women</h1>
          <p class="page-subtitle">Browse all For Women videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('for women', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('for women', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.forwomen-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.forwomen-content { flex: 1; min-width: 0; }
.forwomen-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .forwomen-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .forwomen-page { padding: 24px; } }
@media (max-width: 768px) {
  .forwomen-page { padding: 16px; }
