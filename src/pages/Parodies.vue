    ...
    <div class="category-content">
      <div class="category-page">
        <div class="page-header">
          <h1 class="page-title">Parodies</h1>
          <p class="page-subtitle">Browse entertainment parody content</p>
        </div>
        <SkeletonSection v-if="loading" :count="maxThumbnailsPerPage" :columns="4" />
        <div v-else-if="videos.length > 0" class="videos-content">
          <div class="videos-info"><p class="videos-count">Showing {{ videos.length }} of {{ totalCount }} videos</p></div>
          ...
          <div v-if="totalPages > 1" class="pagination-wrapper">
            ...
            <div class="pagination">
              <button class="pagination-btn" :disabled="currentPage === 1 || loading" @click="goToPage(currentPage - 1)"><ChevronLeft :size="18" /><span>Previous</span></button>
              ...
              <button class="pagination-btn" :disabled="currentPage >= Math.min(totalPages, 1000) || loading" @click="goToPage(currentPage + 1)"><span>Next</span><ChevronRight :size="18" /></button>
            </div>
          </div>
        </div>
        <div v-else class="empty-state"><Video :size="80" class="empty-icon" /><h3>No videos found</h3><p>Try again later</p></div>
      </div>
    </div>
  </div>
</template>
...
const displayTotalPages = computed(() => totalPages.value > 1000 ? '1000+' : totalPages.value.toString());
const visiblePages = computed(() => {
  const pages = [], total = Math.min(totalPages.value, 1000), current = currentPage.value;
...
function goToPage(page) { if (page === '...' || page < 1 || page > totalPages.value) return; searchVideos('parody', page, { order: 'most-popular' }); window.scrollTo({ top: 0, behavior: 'smooth' }); }
onMounted(() => { searchVideos('parody', 1, { order: 'most-popular' }); });
</script>
<style scoped>
.category-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.category-content { flex: 1; min-width: 0; }
.category-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .category-content { margin-left: 0; } }
...
@media (max-width: 768px) { .category-page { padding: 20px; } .page-title { font-size: 1.5rem; } .youtube-videos-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 1rem; } .pagination { flex-wrap: wrap; } }
</style>
