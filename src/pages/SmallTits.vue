<template>
  <div class="smalltits-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="smalltits-content">
      <div class="smalltits-page">
        <div class="page-header">
          <h1 class="page-title">Small Tits</h1>
          <p class="page-subtitle">Browse all Small Tits videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('small tits', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('small tits', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.smalltits-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.smalltits-content { flex: 1; min-width: 0; }
.smalltits-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .smalltits-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .smalltits-page { padding: 24px; } }
@media (max-width: 768px) {
  .smalltits-page { padding: 16px; }
