<template>
  <div class="housewives-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="housewives-content">
      <div class="housewives-page">
        <div class="page-header">
          <h1 class="page-title">Housewives</h1>
          <p class="page-subtitle">Browse all Housewives videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('housewives', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('housewives', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.housewives-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.housewives-content { flex: 1; min-width: 0; }
.housewives-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .housewives-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .housewives-page { padding: 24px; } }
@media (max-width: 768px) {
  .housewives-page { padding: 16px; }
