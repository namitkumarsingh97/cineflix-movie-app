<template>
  <div class="hardcore-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="hardcore-content">
      <div class="hardcore-page">
        <div class="page-header">
          <h1 class="page-title">Hardcore</h1>
          <p class="page-subtitle">Browse all Hardcore videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('hardcore', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('hardcore', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.hardcore-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.hardcore-content { flex: 1; min-width: 0; }
.hardcore-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .hardcore-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .hardcore-page { padding: 24px; } }
@media (max-width: 768px) {
  .hardcore-page { padding: 16px; }
