<template>
  <div class="shemale-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="shemale-content">
      <div class="shemale-page">
        <div class="page-header">
          <h1 class="page-title">Shemale</h1>
          <p class="page-subtitle">Browse all Shemale videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('shemale', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('shemale', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.shemale-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.shemale-content { flex: 1; min-width: 0; }
.shemale-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .shemale-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .shemale-page { padding: 24px; } }
@media (max-width: 768px) {
  .shemale-page { padding: 16px; }
