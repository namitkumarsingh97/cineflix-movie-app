<template>
  <div class="brunette-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="brunette-content">
      <div class="brunette-page">
        <div class="page-header">
          <h1 class="page-title">Brunette</h1>
          <p class="page-subtitle">Browse all Brunette videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('brunette', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('brunette', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.brunette-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.brunette-content { flex: 1; min-width: 0; }
.brunette-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .brunette-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .brunette-page { padding: 24px; } }
@media (max-width: 768px) {
  .brunette-page { padding: 16px; }
