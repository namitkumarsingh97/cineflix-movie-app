...
function goToPage(page) {
  ...
  searchVideos('office', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('office', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.office-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.office-content { flex: 1; min-width: 0; }
.office-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .office-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .office-page { padding: 24px; } }
@media (max-width: 768px) {
  .office-page { padding: 16px; }
