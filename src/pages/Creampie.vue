<template>
  <div class="creampie-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="creampie-content">
      <div class="creampie-page">
        <div class="page-header">
          <h1 class="page-title">Creampie</h1>
          <p class="page-subtitle">Browse all Creampie videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('creampie', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('creampie', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.creampie-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.creampie-content { flex: 1; min-width: 0; }
.creampie-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .creampie-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .creampie-page { padding: 24px; } }
@media (max-width: 768px) {
  .creampie-page { padding: 16px; }
