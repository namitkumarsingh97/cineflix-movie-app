<template>
  <div class="groupsex-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="groupsex-content">
      <div class="groupsex-page">
        <div class="page-header">
          <h1 class="page-title">Group Sex</h1>
          <p class="page-subtitle">Browse all Group Sex videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('group sex', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('group sex', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.groupsex-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.groupsex-content { flex: 1; min-width: 0; }
.groupsex-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .groupsex-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .groupsex-page { padding: 24px; } }
@media (max-width: 768px) {
  .groupsex-page { padding: 16px; }
