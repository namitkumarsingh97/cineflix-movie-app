<template>
  <div class="handjob-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="handjob-content">
      <div class="handjob-page">
        <div class="page-header">
          <h1 class="page-title">Handjob</h1>
          <p class="page-subtitle">Browse all Handjob videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('handjob', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('handjob', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.handjob-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.handjob-content { flex: 1; min-width: 0; }
.handjob-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .handjob-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .handjob-page { padding: 24px; } }
@media (max-width: 768px) {
  .handjob-page { padding: 16px; }
