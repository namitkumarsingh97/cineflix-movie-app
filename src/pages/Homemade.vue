<template>
  <div class="homemade-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="homemade-content">
      <div class="homemade-page">
        <div class="page-header">
          <h1 class="page-title">Homemade</h1>
          <p class="page-subtitle">Browse all Homemade videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('homemade', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('homemade', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.homemade-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.homemade-content { flex: 1; min-width: 0; }
.homemade-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .homemade-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .homemade-page { padding: 24px; } }
@media (max-width: 768px) {
  .homemade-page { padding: 16px; }
