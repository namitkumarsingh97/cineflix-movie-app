<template>
  <div class="lesbian-page-container">
    <CategorySidebar :is-open="sidebarOpen" @filter-change="handleFilterChange" />
    <div class="lesbian-content">
      <div class="lesbian-page">
        <div class="page-header">
          <h1 class="page-title">Lesbian</h1>
          <p class="page-subtitle">Browse all Lesbian videos</p>
        </div>
...
function goToPage(page) {
  ...
  searchVideos('lesbian', page, { order: 'most-popular' });
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
onMounted(() => { searchVideos('lesbian', 1, { order: 'most-popular' }); });
</script>

<style scoped>
.lesbian-page-container { display: flex; position: relative; align-items: flex-start; gap: 0; }
.lesbian-content { flex: 1; min-width: 0; }
.lesbian-page { width: 100%; padding: 40px; margin: 0 auto; max-width: 1600px; }
@media (max-width: 1024px) { .lesbian-content { margin-left: 0; } }
...
@media (max-width: 1024px) { .lesbian-page { padding: 24px; } }
@media (max-width: 768px) {
  .lesbian-page { padding: 16px; }
