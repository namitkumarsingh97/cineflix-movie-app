...
export const paymentVerificationApi = {
  ...
  uploadScreenshot: async (formData) => {
    try {
      const response = await apiClient.post('/payment-verification/upload', formData, {
        headers: {
  ...
  getMyVerifications: async () => {
    try {
      const response = await apiClient.get('/payment-verification/my-verifications');
      return response.data;
    } catch (error) {
  ...
  approveVerification: async (verificationId, notes) => {
    try {
      const response = await apiClient.post(`/payment-verification/${verificationId}/approve`, { notes });
      return response.data;
    } catch (error) {
  ...
  rejectVerification: async (verificationId, rejectionReason, notes) => {
    try {
      const response = await apiClient.post(`/payment-verification/${verificationId}/reject`, {
        rejectionReason,
        notes,
      });
      return response.data;
